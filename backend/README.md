...
| `AGENDA_FILE`             | `agenda.json`                                                                  | Path to the JSON file the agenda is persisted to. Defaults to `agenda.json`, relative to the working directory -- for `go run .` locally that's `backend/`; in the Docker image it's `/data` (the container's `WORKDIR`), so an unset `AGENDA_FILE` resolves to `/data/agenda.json` there. That default path is still not persisted on its own: **production deployments must mount a volume at that path** (e.g. `/data`) so admin edits survive redeploys -- the container itself has no persistent disk otherwise. |
